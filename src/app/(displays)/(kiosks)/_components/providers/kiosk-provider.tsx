...
export const KioskProvider = ({ children, kioskId }: KioskProviderProps) => {
  ...
  const reportState = useCallback(
    (next: Partial<KioskMqttState>) => {
      if (!client || !isConnected) return; // Always check both client and connection
  ...
  useEffect(() => {
    if (!client || !isConnected) return undefined; // Always check both client and connection

    // 1. Load tour command
    // When Docent loads ANY tour, ALL kiosks activate (fetch data, report state)
    // Guard against concurrent loadTour operations
