...
const MetricsSection = ({ metrics, obstacles }: MetricsSectionProps) => {
  return (
    <section className="flex flex-col gap-12 print:gap-4">
      ...
      <div className="grid gap-8 sm:grid-cols-3 print:grid-cols-3 print:gap-5">
      ...
      <div className="rounded-xl border border-[#6DCFF6] bg-white px-6 py-8 shadow-sm sm:px-8 lg:px-10 lg:py-10 print:mt-4 print:px-5 print:py-4">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 print:grid-cols-4 print:gap-3">
          {obstacles.items.map(item => (
            <article className="flex flex-col items-center gap-4 text-center" key={item.title}>
              <span className="rounded-full bg-[#0D3C69] px-3 py-1 text-sm font-semibold text-white print:text-xs">
                {item.title}
              </span>
              <div className="flex h-28 w-28 items-center justify-center print:h-22 print:w-22">
                <div className="flex h-24 w-24 -rotate-45 items-center justify-center rounded-xl border border-[#44A6CC] bg-white print:h-18 print:w-18">
                  <div className="relative flex h-20 w-20 rotate-45 items-center justify-center rounded-full print:h-14 print:w-14">
                    <Image
                      alt=""
                      height={100}
                      loading="eager"
