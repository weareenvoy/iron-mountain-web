...
const MetricsSection = ({ challenges, stats, title, variant = 'default' }: MetricsSectionProps) => {
  ...
  return (
    <section className={`flex flex-col ${isSlide ? 'gap-5' : 'gap-12'}`}>
      <header className={`flex flex-col ${isSlide ? 'gap-1' : 'gap-3'}`}>
        <div
      ...
      <div
        className={`rounded-xl border border-[#6DCFF6] bg-white shadow-sm print:mt-4 print:px-5 print:py-5 ${
        ...
        >
          {challengeItems.map(item => (
            <article className="flex flex-col items-center gap-4 text-center" key={item.title}>
              ...
              <div
                className={`flex items-center justify-center print:h-24 print:w-24 ${isSlide ? 'h-24 w-24' : 'h-28 w-28'}`}
              >
                <div
                  className={`flex items-center justify-center rounded-xl border border-[#44A6E6] bg-white print:h-20 print:w-20 ${
                    isSlide ? 'h-20 w-20 -rotate-45' : 'h-24 w-24 -rotate-45'
                  }`}
                >
                  <div
                    className={`relative flex rotate-45 items-center justify-center rounded-full print:h-16 print:w-16 ${
              ...
              <p className={`text-[#58595B] ${isSlide ? 'text-sm' : 'text-sm'}`}>{item.body}</p>
            </article>
          ))}
        </div>
      </div>
